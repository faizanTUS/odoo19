...
class AccountPayment(models.Model):
    ...
    def action_open_destination_journal(self):
        """Open the destination journal form (used from the payment form warning)."""
        self.ensure_one()
    ...
    def _get_aml_default_display_name_list(self):
        self.ensure_one()
        values = list(super()._get_aml_default_display_name_list())
        if self.is_internal_transfer:
            if values:
                values[0] = ('label', _("Internal Transfer"))
            else:
                values.append(('label', _("Internal Transfer")))
        return values
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        write_off_amount_currency = sum(x['amount_currency'] for x in write_off_line_vals)
        write_off_balance = sum(x['balance'] for x in write_off_line_vals)
    ...
    def _get_trigger_fields_to_synchronize(self):
        ...
        if 'is_internal_transfer' in fields:
            return fields
        return fields + ('is_internal_transfer',)
    ...
    def copy_data(self, default=None):
        ...
        if self.env.context.get('internal_transfer'):
