...
patch(PosStore.prototype, {
    async activateCode(code) {
        ...
        if (rule) {
            ...
            if (successful) {
               // var {confirmed, amount:amount } = await makeAwaitable(this.dialog, PromoCodePopups, {
               var dialogResult = await makeAwaitable(this.dialog, PromoCodePopups, {
                    title: _t("Redeem Amount"),
                    placeholder: _t("Enter amount to Redeem"),
                    code: code,
                    partner_id: order.get_partner() ? order.get_partner().name : '',
                    points: payload.points,
                });
               if (!dialogResult || !dialogResult.confirmed) {

                    return _t("Redemption cancelled.");

                }

                var {confirmed, amount:amount } = dialogResult;
