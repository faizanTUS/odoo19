...
class ResUsers(models.Model):
    ...
    #     _logger.info("Login credential", credential)
    #     ip = request.httprequest.environ['REMOTE_ADDR'] if request else 'n/a'
    #
    #     if request.params.get('otp'):
    #         _logger.info("Login otppppppppppp ", request.params.get('otp'))
    #         try:
    #             with cls.pool.cursor() as cr:
    #                 self = api.Environment(cr, SUPERUSER_ID, {})[cls._name]
    #                 with self._assert_can_auth():
        ...
        try:
            with cls.pool.cursor() as cr:
                ...
                with self._assert_can_auth(user=login):
                    ...
                    # 🔐 OTP LOGIN
                    if request and request.session.get('login_by_otp'):
                        _logger.info("OTP login detected for %s", login)

                        sid = request.session.sid  # 🔑 REQUIRED IN ODOO 18

                        auth_info = {
                            'uid': user.id,
                            'login': user.login,
                            'session_token': user._compute_session_token(sid),
                        }
            ...
            _logger.info(
                "Login failed for db:%s login:%s from %s",
                db, login, ip
            )
            raise

        _logger.info(
            "Login successful for db:%s login:%s from %s",
            db, login, ip
        )
        return auth_info