...
class PendingPaymentReportLineDetail(models.TransientModel):
    ...
    @api.depends("move_id", "move_id.reconciled_payment_ids", "move_id.reconciled_payment_ids.date")
    def _compute_payment_dates(self):
        for rec in self:
            if not rec.move_id or not rec.move_id.reconciled_payment_ids:
                rec.payment_dates = ""
                continue
            dates = rec.move_id.reconciled_payment_ids.mapped("date")
            rec.payment_dates = ", ".join(str(d) for d in sorted(dates))
