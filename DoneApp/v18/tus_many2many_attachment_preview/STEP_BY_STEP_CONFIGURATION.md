...
`odoo18/project/tus_many2many_attachment_preview/`
...
1. Confirm the module folder exists on disk, for example:  
   `/home/tus/workspace/odoo18/project/tus_many2many_attachment_preview/`
...
3. Locate **`addons_path`**. Add the **`project`** directory (the parent folder that contains `tus_many2many_attachment_preview`), **not** only the module folder.
   ...
   Adjust paths to match your server. The important part is that **`.../odoo18/project`** is listed so Odoo can discover **`tus_many2many_attachment_preview`** inside it.
