...
class ExportDeliveryPDFWizard(models.TransientModel):
    _name = 'export.delivery.pdf.wizard'
    _description = 'Export Delivery PDF Wizard'
    ...
    def action_delivery_report(self):
        return self._export_action(
            'Receipts(Incoming)Deliveries(Outgoing).zip',
            self.picking_ids.filtered(lambda p: p.picking_type_id.code in ['incoming', 'outgoing']),
            'stock.action_report_delivery',
        )

    def action_incoming_delivery(self):
        return self._export_action(
            'Receipts (Incoming ).zip',
