...
class EmployeeImageExportWizard(models.TransientModel):
    ...
    def _process_image(self, raw_bytes):
        """Convert, resize, and return (processed_bytes, extension), or (None, None) on skip."""
        if not raw_bytes:
            return None, None

        output_format = (self.output_format or "original").lower()
        needs_pil = self.resize_enabled or output_format in ("png", "jpeg")

        if not needs_pil and output_format == "original":
            return raw_bytes, "png"

        if Image is None:
            raise UserError(
                _("Pillow (PIL) is required for resizing or format conversion. "
                  "Install Pillow or use original format without resize.")
            )
        ...
        if self.resize_enabled and (self.resize_width or self.resize_height):
            try:
                tw = self.resize_width or img.width
                th = self.resize_height or img.height
                if self.keep_ratio:
                    img.thumbnail((tw, th))
                else:
        ...
        if str(img_format).upper() == "JPEG" and img.mode in ("RGBA", "P"):
            try:
                img = img.convert("RGB")
            except Exception as e:
                _logger.warning("Failed to convert image for JPEG: %s", e)
                return None, None
