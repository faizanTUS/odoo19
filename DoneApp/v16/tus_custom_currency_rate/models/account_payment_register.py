...
class AccountPaymentRegister(models.TransientModel):
                ...
                if len(record_id) == 1:
                    if (
                        ...
                        if record_id.company_id.currency_id != rec.currency_id:
                            rec.currency_rate = record_id.new_currency_rate
                            rec.manual_currency_rate = record_id.new_currency_rate
    ...
    def default_get(self, fields):
        ...
        if self._context.get("active_model") and self._context.get("active_ids"):
            ...
            if len(record_id) == 1:
                vals["manual_currency_rate"] = record_id.new_currency_rate
                vals["currency_rate"] = record_id.new_currency_rate
                vals[
                    "currency_rate_for_invoice_currency_to_company_currency"
                ] = record_id.new_currency_rate
            vals["is_multiple_record"] = len(record_id) > 1 and True or False
            vals["currency_rate_for_multiple_records"] = record_id[:1].new_currency_rate
            vals["invoice_ids"] = [(6, 0, record_id.ids)]
        return vals
