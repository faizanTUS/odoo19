...
class PosOrderCancelWizard(models.TransientModel):
    ...
    def _cancel_done_picking_impl(self, picking, StockQuant):
        """
        HARD force cancel of DONE picking in Odoo 16.
        - Reverse stock manually
        - Reset move state to draft to bypass unlink protection
        - Unlink move lines
        - Force cancel move and picking
        """
        ...
        for move in picking.move_ids.filtered(lambda m: m.state == 'done' and not m.scrapped):
            ...
            for ml in move.move_line_ids:
                ...
                # Remove quantity from destination location
                ...
                # Add quantity back to source location
                StockQuant._update_available_quantity(
                    ml.product_id,
                    ml.location_id,
                    ml.qty_done,
            ...
            # 🔥 VERY IMPORTANT PART
            # Reset move to draft first to bypass ondelete protection
            move.sudo().write({'state': 'draft'})

            # Now unlink move lines safely
            move.move_line_ids.sudo().unlink()

            # Finally cancel the move
            move.sudo().write({'state': 'cancel'})

        # Force cancel picking
