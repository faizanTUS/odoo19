...
class ExportPDFWizard(models.TransientModel):
    ...
    def action_purchase_order(self):
        return self._export_action('Purchase Order.zip', self.purchase_order_ids.filtered(lambda p: p.state in ['to_approve','purchase']),
                                   'purchase.action_report_purchase_order')

    def action_purchase_quotation(self):
        return self._export_action('Purchase Quotation.zip',self.purchase_order_ids.filtered(lambda p: p.state in ['draft', 'sent']),
                                   'purchase.action_report_purchase_order')
