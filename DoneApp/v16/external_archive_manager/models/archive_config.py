    ...
    def cron_master_scheduler(self):
        ...
        for cfg in configs:
            ...
            if cfg.offload_mode in ("scheduled", "both"):
                ...
                # if not cfg.next_offload_run_at or cfg.next_offload_run_at <= now:
                #     try:
                #         offloaded = cfg._run_scheduled_offload()
                #         cfg.last_offload_run_at = now
                #         cfg.next_offload_run_at = cfg._compute_next_offload_run()
                #         self.env["external.archive.log"].sudo().create(
                #             {"action": "offload", "model": cfg.model_name, "count": offloaded, "status": "success", "message": "Cron offload executed"}
                #         )
                #     except Exception as e:
                #         self.env["external.archive.log"].sudo().create(
                #             {"action": "offload", "model": cfg.model_name, "status": "failed", "message": str(e)}
                #         )
            ...
            if cfg.delete_mode in ("scheduled", "both"):
                ...
                # if not cfg.next_run_at or cfg.next_run_at <= now:
                #     try:
                #         deleted = cfg._run_scheduled_delete()
                #         cfg.last_run_at = now
                #         cfg.next_run_at = cfg._compute_next_run()
                #         self.env["external.archive.log"].sudo().create(
                #             {"action": "cron_delete", "model": cfg.model_name, "count": deleted, "status": "success", "message": "Cron delete executed"}
                #         )
                #     except Exception as e:
                #         self.env["external.archive.log"].sudo().create(
                #             {"action": "cron_delete", "model": cfg.model_name, "status": "failed", "message": str(e)}
                #         )