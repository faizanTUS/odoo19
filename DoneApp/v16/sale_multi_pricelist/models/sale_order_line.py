...
class SaleOrderLine(models.Model):
    ...
    @api.depends('product_id', 'product_uom', 'product_uom_qty', 'pricelist_id', 'order_id.pricelist_id')
    ...
    @api.depends('product_id', 'product_uom', 'product_uom_qty', 'pricelist_id')
    ...
    def _get_display_price(self):
        """Override to use effective pricelist discount policy instead of order pricelist."""
        self.ensure_one()

        # If line has its own pricelist, use effective pricelist discount policy
        if self.pricelist_id:
            pricelist_price = self._get_pricelist_price()
            effective_pricelist = self._get_pricelist_for_line()

            if effective_pricelist.discount_policy == 'with_discount':
        ...
        # No line pricelist set — fall back to standard Odoo behavior
...
class ProductPricelist(models.Model):
    ...
    def name_get(self):
        ...
        if self.env.context.get('hide_currency_in_name'):
            return [(pl.id, pl.name) for pl in self]
