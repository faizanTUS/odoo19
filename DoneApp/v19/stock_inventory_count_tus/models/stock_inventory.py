...
class Inventory(models.Model):
    ...
    def action_view_count_stock_move_lines(self):
        ...
        if self.product_ids:
            ...
            move_domain = Domain.AND([move_domain, product_ids])
    ...
    def _get_quantities(self):
        ...
        if self.product_ids:
            ...
            domain = Domain.AND([domain, product_ids])
            move_domain = Domain.AND([move_domain, product_ids])
        if self.from_date_range:
            domain = Domain.AND([domain, [("in_date", ">=", self.from_date_range)]])
            move_domain = Domain.AND(
                [move_domain, [("date", ">=", self.from_date_range)]]
            )
        if self.to_date_range:
            domain = Domain.AND([domain, [("in_date", "<=", self.to_date_range)]])
            move_domain = Domain.AND(
                [move_domain, [("date", "<=", self.to_date_range)]]
            )
        fields = [
