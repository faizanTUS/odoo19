...
class AdvancedAccessService(models.AbstractModel):
    ...
    def _rules_payload_json(self):
        ...
        su = user.sudo()
        # Superuser, Settings admins, ERP managers, AAC managers: no restrictions (UI + server)
        if user._is_superuser() or su._is_system() or su.has_group(
            "base.group_erp_manager"
        ) or su.has_group("advanced_access_control.group_advanced_access_manager"):
            return json.dumps({"empty": True})
        ...
        for pol in policies:
            ...
            for line in pol.model_line_ids:
                ...
                if not line.allow_sidebar_cancel:
                    sid |= _aac_binding_action_ids_matching(self.env, line.model_name, "cancel")
                if not line.allow_sidebar_send_email:
                    sid |= _aac_binding_action_ids_matching(self.env, line.model_name, "email")
                if sid:
                    hidden_sidebar_action_ids_by_model.setdefault(line.model_name, set()).update(sid)
                if line.hidden_sidebar_action_ids:
                    hidden_sidebar_action_ids_by_model.setdefault(line.model_name, set()).update(line.hidden_sidebar_action_ids.ids)
        ...
        payload = {
            ...
            "model_ui": model_ui,
            "hidden_reports_by_model": {k: sorted(v) for k, v in hidden_reports_by_model.items()},
            "hidden_sidebar_action_ids_by_model": {k: sorted(v) for k, v in hidden_sidebar_action_ids_by_model.items()},
        }
    ...
    def _audit_denial(self, model_name, operation, detail):
        ...
        if not rules or not rules.get("audit"):
            return
        try:
            pol = self.env["advanced.access.policy"].browse(rules.get("policy_ids", [])[:1])
            self.env["advanced.access.audit.log"].sudo().create(
                {
                    "policy_id": pol.id if pol else False,
