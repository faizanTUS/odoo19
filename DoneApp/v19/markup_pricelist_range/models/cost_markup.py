    ...
    def _check_cost_range(self):
        for rule in self:
            if rule.min_cost > rule.max_cost:
                raise ValidationError(_("Min Cost cannot be greater than Max Cost."))
            ...
            if overlapping_rules:
                raise ValidationError(
                    _("Overlapping pricing rules detected. Please adjust the cost range.")
                )
    ...
    def _compute_price(self, product, quantity, uom, date, currency=None, **kwargs):
        ...
        if not pricelist:
            pricelist = self.env['product.pricelist'].browse(
                self.env.user.partner_id.property_product_pricelist.id
            )
        res = super()._compute_price(product, quantity, uom, date, currency, **kwargs)
