...
class AccountMove(models.Model):
    ...
    def get_views(self, views, options=None):
        ...
        # vendor payment
        # elif res['views'].get('list') and res['views']['list']['id'] == self.env.ref('account.view_account_supplier_payment_tree').id:
        #     if toolbar := res['views']['list'].get('toolbar'):
        #         actions_to_remove = []
        #         for action in toolbar.get('action', []):
        #             if action.get('id') in [invoice_action.id, bill_action.id, customer_credit_note_action.id,vendor_credit_note_action.id,customer_payment_action.id]:
        #                 actions_to_remove.append(action)
        #         for action in actions_to_remove:
        #             toolbar['action'].remove(action)
