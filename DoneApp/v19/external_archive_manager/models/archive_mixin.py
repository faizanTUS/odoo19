...
class ExternalArchiveMixin(models.AbstractModel):
    ...
    def _external_offload_one(self):
        ...
        try:
            ...
            conn.rollback()
            # self.env["external.archive.log"].sudo().create(
            #     {"action": "offload", "model": self._name, "res_id": self.id, "status": "failed", "message": str(e)}
            # )
        ...
        # # Tracking upsert
        # Tracking = self.env["external.archive.tracking"].sudo()
        # now = fields.Datetime.now()
        # for fname, _ftype in fields_to_archive:
        #     tr = Tracking.search([("model", "=", self._name), ("res_id", "=", self.id), ("field_name", "=", fname)], limit=1)
        #     vals = {"archived_at": now, "archived_by": self.env.user.id, "state": "archived"}
        #     if tr:
        #         tr.write(vals)
        #     else:
        #         vals.update({"model": self._name, "res_id": self.id, "field_name": fname})
        #         Tracking.create(vals)
        #
        # self.env["external.archive.log"].sudo().create(
        #     {"action": "offload", "model": self._name, "res_id": self.id, "count": len(fields_to_archive), "status": "success"}
        # )
        return True
    ...
    def _external_retrieve_one(self):
        ...
        self.write(write_vals)
        #
        # self.env["external.archive.tracking"].sudo().search([("model", "=", self._name), ("res_id", "=", self.id)]).write(
        #     {"state": "restored"}
        # )

        # self.env["external.archive.log"].sudo().create(
        #     {"action": "retrieve", "model": self._name, "res_id": self.id, "count": len(rows), "status": "success"}
        # )
        return True
    ...
    def _external_delete_one(self):
        ...
        try:
            ...
            conn.rollback()
            # self.env["external.archive.log"].sudo().create(
            #     {"action": "delete", "model": self._name, "res_id": self.id, "status": "failed", "message": str(e)}
            # )
        ...
        #
        # self.env["external.archive.log"].sudo().create(
        #     {"action": "delete", "model": self._name, "res_id": self.id, "count": deleted, "status": "success"}
        # )
        return True