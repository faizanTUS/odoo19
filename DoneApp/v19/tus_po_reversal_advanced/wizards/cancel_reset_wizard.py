...
class TusCancelResetWizard(models.TransientModel):
    ...
    def _cancel_po(self, po):
        """
        Safe cancellation:
        - Reverse pickings first
        - Do NOT cancel original receipt (prevent Odoo from auto-cancelling the return)
        - Cancel bills
        - Cancel PO
        """
        ...
        if self.include_dependencies:
            ...
            for picking in po.tus_get_related_pickings().filtered(lambda p: p.state == "done"):
            ...
            # Do NOT cancel original pickings
            # We keep them in 'done' to avoid auto-cancel on return
            # (Odoo cancels returns if the origin is cancelled)
            pass

        if po.state not in ("cancel", "done"):
            po.with_context(disable_cancel_warning=True).write({'state': 'cancel'})
