...
class BulkAttachmentsZipWizard(models.TransientModel):
    ...
    def action_refresh_lines(self):
        ...
        self.env[self.res_model].browse(ids).exists().check_access("read")
        ...
        # Odoo 18: returning False from call_button becomes act_window_close and closes the
        # dialog (user lands on the list behind). Re-open the same wizard so the modal stays.
        return {
