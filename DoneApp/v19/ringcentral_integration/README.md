...
### 📝 Call Data
- **Call Transcripts**: Fetch transcripts via RingSense (priority) or Speech-to-Text (fallback) when recording is available
- **Transcript Source**: RingSense provides rich AI insights (speaker names, summaries); Speech-to-Text provides basic transcription
...
4. Enable the following permissions:
   ...
   - **Read RingSense Insights** (optional, for RingSense transcripts)
   - **Audio & Video AI** (optional, for Speech-to-Text fallback)
...
- **Transcripts**: Click "Fetch Transcript" on a call with a recording. RingSense is tried first (requires license); if unavailable, Speech-to-Text runs asynchronously (check back in a few minutes)
...
- `transcript_source` - ringsense or speech_to_text (method used)
- `transcript_job_id` - Speech-to-Text job ID when transcription is in progress
...
- **No recording**: Transcripts require a call recording. Ensure the call was recorded.
- **RingSense 404**: RingSense requires a license. If unavailable, the module falls back to Speech-to-Text automatically.
- **Speech-to-Text in progress**: Transcription is async. A cron runs every 5 minutes to poll and update. Refresh the call record after a few minutes.
- **Permission errors**: Enable "Read RingSense Insights" and "Audio & Video AI" in your RingCentral app if you see permission errors.
