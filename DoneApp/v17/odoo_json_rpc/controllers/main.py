...
class OdooAPI(DataSet):
    ...
    def user_authenticate(self, **post):
        ...
        res = {'token': get_user_token.token, 'uid': uid, 'company_ids': user.company_ids.ids,'partner_id': user.partner_id.id}
        if get_admin_token:
                       res.update({
    ...
    def jsonrpc_method(self, **post):
        ...
        res = self._call_kw(model, method, post.get('args') or [[]], post.get('context') or {})
