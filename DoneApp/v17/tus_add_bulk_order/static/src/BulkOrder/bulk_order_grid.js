...
export class BulkOrderGridComponent extends Component {
    ...
    onQuantityChange(ev) {
        const rowIndex = parseInt(ev.target.dataset.rowIndex, 10);
        const variantIndex = parseInt(ev.target.dataset.variantIndex, 10);
    ...
    async onSearchInput(ev) {
        const index = parseInt(ev.target.dataset.rowIndex, 10);
    ...
    selectProductFromEvent(ev) {
        const rowIndex = parseInt(ev.currentTarget.dataset.index, 10);
    ...
    calculateRowTotals(row) {
        ...
        const rounded = parseFloat(lineTotal.toFixed(2));
    ...
    onRemoveRowClick(ev) {
        const index = parseInt(ev.currentTarget.dataset.rowIndex, 10);
        this.clearRow(this.rows[index]);
    }

    async onAddRowToSaleOrder(ev) {
        const rowIndex = parseInt(ev.currentTarget.dataset.rowIndex, 10);
        const row = this.rows[rowIndex];
