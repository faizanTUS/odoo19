...
function isInListView(target) {
    ...
    // Check for Odoo 18 list view structure - multiple selectors
...
function getCell(target) {
    ...
    // Odoo 18 uses .o_data_cell or td elements
...
function getFocusableInCell(cell) {
    ...
    // In Odoo 18, inputs might be in o_field_widget divs
    const candidates = cell.querySelectorAll(FOCUSABLE_SELECTOR);
    for (const el of candidates) {
        if (el.disabled || el.readOnly) continue;
        // Skip hidden inputs
        if (el.type === "hidden") continue;
        // In Odoo 18, inputs might not be visible until cell is in edit mode
...
function focusCellInRowByColumn(row, colIndex, placeCaretAtEnd = false) {
    ...
    // In Odoo 18, cells need to be clicked to enter edit mode
