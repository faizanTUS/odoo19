...
class PosOrder(models.Model):
    ...
    def _order_fields(self, ui_order):
        ...
        order_fields['rounding_id'] = ui_order.get('rounding_id')
        return order_fields

    def _export_for_ui(self, order):
        result = super(PosOrder, self)._export_for_ui(order)
        result['rounding_id'] = order.rounding_id.id
        return result


    def action_pos_order_paid(self):
        self.ensure_one()
        if not self.rounding_id:
            ...
            if not self.config_id.cash_rounding \
                ...
                total = float_round(self.amount_total,
                                    precision_rounding=self.rounding_id.rounding,
                                    rounding_method=self.rounding_id.rounding_method)
            ...
            if not isPaid and not self.config_id.cash_rounding:
                ...
                currency = self.currency_id
                if self.rounding_id.rounding_method == "HALF-UP":
                    maxDiff = currency.round(self.rounding_id.rounding / 2)
                else:
                    maxDiff = currency.round(self.rounding_id.rounding)
