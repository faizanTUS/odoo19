...
def _attendance_has_date_field(env):
    ...
    runtime."""
    Att = env.get("hr.attendance") if env is not None else None
    return Att is not None and "date" in Att._fields
...
def cockpit_leave_company_domain(env, company_ids):
    field_name = "company_id" if "company_id" in env["hr.leave"]._fields else "employee_company_id"
    return ["|", (field_name, "=", False), (field_name, "in", company_ids)]


def cockpit_leave_overlap_domain(env, date_from, date_to, company_ids):
    """Leaves whose request window overlaps [date_from, date_to]."""
    return (
        cockpit_leave_company_domain(env, company_ids)
        + [
