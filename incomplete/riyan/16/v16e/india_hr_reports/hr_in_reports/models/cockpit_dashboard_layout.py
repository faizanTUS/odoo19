...
def collect_dashboard_layout(env, filters):
    ...
    if cockpit_module_installed(env, "hr_holidays"):
        Leave = env["hr.leave"]
        leave_today_dom = (
            cockpit_leave_overlap_domain(env, today, today, companies)
    ...
    if cockpit_module_installed(env, "hr_holidays"):
        Leave = env["hr.leave"]
        leave_period_dom = cockpit_leave_overlap_domain(env, date_from, date_to, companies) + cockpit_hr_filters_leave_domain(
            filters, env=env
        )
