...
import { routeToUrl, parseSearchQuery } from "@web/core/browser/router_service";
...
/**
 * Odoo 17: open full list in a new tab via window action id (hash URL).
 */
function urlToOpenFullListActionInNewTab(actionId, router) {
    const hash = { action: actionId };
    if (router?.current?.hash?.cids != null && router.current.hash.cids !== "") {
        hash.cids = router.current.hash.cids;
    }
    return (
        browser.location.origin +
        routeToUrl({
            pathname: browser.location.pathname,
            search: parseSearchQuery(browser.location.search),
            hash,
        })
    );
}
...
/**
 * Odoo 17: hash URL for opening a record form in a new tab.
 */
function urlToOpenRecordInNewTab(model, resId, router) {
    const hash = {
        id: Number(resId),
        model,
        view_type: "form",
...
export class GlobalSearchPanel extends Component {
    ...
    async loadConfigIfNeeded(force = false) {
        ...
        try {
            const cfg = await this.rpc("/tus_smart_global_search/config", {});
    ...
    async doSearch() {
        ...
        try {
            const { groups } = await this.rpc("/tus_smart_global_search/search", { query });
    ...
    async loadBrowse(suggestion) {
        ...
        try {
            ...
            const { groups } = await this.rpc("/tus_smart_global_search/browse", payload);
            if (reqId !== this._reqId) {
                return;
            }
            this.state.groups = groups || [];
    ...
    async openFullListFromBrowse() {
        ...
        try {
            const nav = await this.rpc("/tus_smart_global_search/browse_list_nav", {
            ...
            if (!Number.isFinite(actionId)) {
                const fallback = await this.rpc("/tus_smart_global_search/browse_action", {
                    model: this.state.browseModel,
                    browse_preset: this.state.browsePreset || null,
                });
            ...
            const openUrl = urlToOpenFullListActionInNewTab(actionId, this.router);
            const w = window.open(openUrl, "_blank");
            if (!w) {
                const action = await this.rpc("/tus_smart_global_search/browse_action", {
                    model: this.state.browseModel,
                    browse_preset: this.state.browsePreset || null,
                });
    ...
    async openRecord(model, result) {
        ...
        try {
            let action = await this.rpc("/tus_smart_global_search/open_record_action", {
                model,
                res_id: rid,
            });
            if (!action || !action.type) {
    ...
    async openRecordInNewTab(model, result, ev) {
        ...
        addRecentSearch(this.state.query);
        this.state.frequent = recordFrequentAccess(model, result).slice(0, FREQUENT_SHOW);
        const openUrl = urlToOpenRecordInNewTab(model, result.id, this.router);
...
export class GlobalSearchSystray extends Component {
    ...
    async _loadConfig() {
        try {
            const result = await this.rpc("/tus_smart_global_search/config", {});
