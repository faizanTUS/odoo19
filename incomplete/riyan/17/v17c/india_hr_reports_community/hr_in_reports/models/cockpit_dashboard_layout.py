...
def collect_dashboard_layout(env, filters):
    ...
    # ``Claims summary`` is the shortest card; appending it to col1 (which is
    # otherwise one card shorter than col2) keeps the two main columns at
    # roughly the same height and avoids the large blank gap that used to
    # appear below the shorter column.
    if cockpit_module_installed(env, "hr_expense"):
        col1.append(
            {
                "id": "claims",
