...
class HrInReportWizardMixin(models.AbstractModel):
    ...
    def _max_export_rows(self):
        param = self.env["ir.config_parameter"].sudo().get_param(
            "india_hr_reports_community.max_export_rows", "50000"
        )
        try:
    ...
    def action_export_pdf(self):
        ...
        if not xmlid:
            raise UserError(_("PDF report is not configured for this wizard."))
        report = self.env.ref(xmlid, raise_if_not_found=False)
        if not report:
            raise UserError(_("Missing PDF report definition."))
