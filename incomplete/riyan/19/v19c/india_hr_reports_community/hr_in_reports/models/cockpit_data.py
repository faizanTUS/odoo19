...
def _attendance_has_date_field(env):
    """``hr.attendance.date`` is a computed/stored field added in Odoo 19; on
    Odoo 18 / 17 / 16 the model only has ``check_in`` (Datetime). We keep the
    cockpit code version-agnostic by introspecting the model's fields at
    runtime. We also assert the field type so a third-party module that
    declares a non-date ``date`` attribute on ``hr.attendance`` cannot trick
    us into building an invalid domain."""
    if env is None or "hr.attendance" not in env:
        return False
    fld = env["hr.attendance"]._fields.get("date")
    return bool(fld) and fld.type in ("date", "datetime")


def cockpit_attendance_date_range_domain(env, date_from, date_to):
    """Domain leaves selecting hr.attendance lines whose date falls in
    ``[date_from, date_to]`` inclusive. Uses the v19 ``date`` field when
    present, otherwise falls back to a half-open ``check_in`` datetime range
    so the last day is included even on older versions."""
    if _attendance_has_date_field(env):
        return [("date", ">=", date_from), ("date", "<=", date_to)]
    dt_from = datetime.combine(date_from, datetime.min.time())
    dt_to_next = datetime.combine(date_to + timedelta(days=1), datetime.min.time())
    return [("check_in", ">=", dt_from), ("check_in", "<", dt_to_next)]


def cockpit_attendance_today_domain(env, today):
    """Domain leaves selecting hr.attendance lines for the given calendar day."""
    if _attendance_has_date_field(env):
        return [("date", "=", today)]
    dt_start = datetime.combine(today, datetime.min.time())
    dt_end = datetime.combine(today + timedelta(days=1), datetime.min.time())
    return [("check_in", ">=", dt_start), ("check_in", "<", dt_end)]


def cockpit_attendance_day_groupby(env):
    """Group-by spec used to aggregate hr.attendance per calendar day."""
    return "date:day" if _attendance_has_date_field(env) else "check_in:day"
...
def cockpit_hr_filters_expense_domain(filters, env=None):
    ...
    ``hr.expense.department_id`` is a stored field on Odoo 19; on Odoo 18 / 17
    / 16 it does not exist and we have to traverse via ``employee_id``.
    """
    fc = filters or {}
    dom = list(_cockpit_hierarchy_domain(env))

    Exp = env.get("hr.expense") if env is not None else None
    dept_field = (
        "department_id"
        if Exp is not None and "department_id" in Exp._fields
        else "employee_id.department_id"
    )
