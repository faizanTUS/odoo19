...
class PayrollCockpitProvider:
    ...
    def collect(env, filters):
        ...
        by_state = Slip.formatted_read_group(dom, ["state"], ["__count"])
        ...
        for row in sorted(by_state, key=lambda r: r.get("state") or ""):
            ...
            labels.append(state_labels.get(code, code or env._("Unknown")))
            ...
            charts.append(
                {
                    "id": "payroll_by_state",
                    "type": "bar",
                    "title": env._("Confirmed payslips by status"),
                    "labels": labels,
                    "datasets": [{"label": env._("Payslips"), "data": counts}],
                }
            )
        ...
        return {
            ...
            "kpis": [
                {
                    "key": "pay_confirmed",
                    "label": env._("Confirmed payslips"),
                    "value": total,
                    "hint": env._("Validated / paid in period window"),
                },
                {
                    "key": "pay_draft",
                    "label": env._("Draft payslips"),
                    "value": draft_count,
                    "hint": env._("Overlapping period"),
                },
            ],
            "charts": charts,
        }
