...
def cockpit_satisfaction_payload(env, date_from, date_to, companies, emp_dom):
    ...
    if cockpit_module_installed(env, "survey") and "survey.user_input" in env:
        ...
        rows = Inp.formatted_read_group(
            dom, [], ["scoring_percentage:avg", "__count"]
        )
        if rows:
            avg_pct = rows[0].get("scoring_percentage:avg") or 0
    ...
    if cockpit_module_installed(env, "hr_appraisal") and env.registry.get(
        ...
        if n >= 2:
            ...
            rows = Appraisal.formatted_read_group(
                app_dom, ["assessment_note"], ["__count"]
            )
...
def _views_from_view_mode(view_mode):
    ...
    raw = (view_mode or "list,form").strip()
...
def cockpit_act_window(name, model, domain, view_mode="list,form", group_by=None, context_extra=None):
...
def collect_dashboard_layout(env, filters):
    ...
    if cockpit_module_installed(env, "hr_attendance"):
        ...
        emps_today = Att.formatted_read_group(att_today_dom, ["employee_id"], ["__count"])
    ...
    if cockpit_module_installed(env, "hr_payroll") and "hr.payslip" in env:
        ...
        if slips and env.registry.get("hr.payslip.line"):
            ...
            rows = Line.formatted_read_group(
    ...
    if cockpit_module_installed(env, "hr_holidays"):
        ...
        rows = Leave.formatted_read_group(dom, ["state"], ["__count"])
    ...
    if cockpit_module_installed(env, "hr_recruitment"):
        ...
        rows = App.formatted_read_group(app_dom, ["stage_id"], ["__count"])
    ...
    if cockpit_module_installed(env, "hr_expense"):
        ...
        rows = Exp.formatted_read_group(exp_dom, ["state"], ["total_amount:sum"])
        state_labels = dict(Exp._fields["state"]._description_selection(env))
        for row in sorted(rows, key=lambda r: -(r.get("total_amount:sum", 0) or 0)):
            code = row.get("state") or ""
            claims_list.append(
                {
                    "label": state_labels.get(code, code or _("Unknown")),
                    "value": format_amount(env, row.get("total_amount:sum") or 0, currency),
                }
            )
