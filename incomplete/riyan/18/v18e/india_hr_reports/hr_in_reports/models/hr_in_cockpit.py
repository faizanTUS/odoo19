        ...
        try:
            ...
            for key, provider_cls in iter_cockpit_providers():
                ...
                try:
                    ...
                    _logger.exception(
                        "HR Cockpit: segment provider [%s] failed with filters %s: %s",
                        key, filters, seg_err
                    )
            ...
            try:
                ...
                _logger.exception(
                    "HR Cockpit: collect_dashboard_layout failed with filters %s: %s",
                    filters, layout_err
                )
                raise
            return {
            ...
            _logger.exception(
                "HR Cockpit: get_dashboard_payload failed with filters %s", filters
            )
            raise
