...
def cockpit_satisfaction_payload(env, date_from, date_to, companies, emp_dom):
    ...
    if cockpit_module_installed(env, "survey") and "survey.user_input" in env:
        ...
        rows = Inp.read_group(
            dom, ["scoring_percentage:avg"], [], lazy=False,
        )
        if rows:
    ...
    if cockpit_module_installed(env, "hr_appraisal") and env.registry.get(
        ...
        if n >= 2:
            ...
            rows = Appraisal.read_group(
                app_dom, [], ["assessment_note"], lazy=False,
            )
...
def collect_dashboard_layout(env, filters):
    ...
    if "employee_type" in Emp._fields:
        rows = Emp.read_group(
            emp_dom + [("active", "=", True), ("employee_type", "!=", False)],
            [],
            ["employee_type"],
            lazy=False,
        )
        labels = []
        data = []
        type_labels = dict(Emp._fields['employee_type']._description_selection(env))
        for row in sorted(rows, key=lambda r: -(r.get("__count", 0) or 0)):
            ct = row.get("employee_type")
            labels.append(type_labels.get(ct, ct or _("Type")))
    ...
    if cockpit_module_installed(env, "hr_appraisal") and env.registry.get("hr.appraisal"):
        ...
        app_dom += cockpit_hr_filters_attendance_domain(filters, env=env)
        rows = Appraisal.read_group(
            app_dom, [], ["assessment_note"], lazy=False,
        )
    ...
    workforce_footer_action = cockpit_employee_window_action(
        ...
        emp_active_dom,
        ["employee_type"] if "employee_type" in Emp._fields else [],
    )
