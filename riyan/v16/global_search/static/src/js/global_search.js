...
export class GlobalSearchPanel extends Component {
    ...
    async doSearch() {
        ...
        try {
            const { groups } = await this.rpc("/global_search/search", { query });
...
export class GlobalSearchSystray extends Component {
    ...
    async _loadConfig() {
        try {
            const result = await this.rpc("/global_search/config", {});
...
GlobalSearchSystray.template = "global_search.GlobalSearchSystray";
