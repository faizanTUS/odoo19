...
class BulkAttachmentsZipWizard(models.TransientModel):
    ...
    def _commands_for_lines(self, res_model, res_ids, include_chatter):
        ...
        # Odoo 16 ir.attachment search injects res_field=False when the domain omits
        # res_field; merge a second search for res_field != False.
        Attachment = self.env["ir.attachment"]
        base = [("res_model", "=", res_model), ("res_id", "in", res_ids)]
        atts = Attachment.search(base)
        atts |= Attachment.search(base + [("res_field", "!=", False)])
        seen = set(atts.ids)
        commands = [
            (
                0,
