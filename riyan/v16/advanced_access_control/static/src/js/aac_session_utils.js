...
/**
 * Hidden form button XML names for this model.
 * @param {string} resModel
 * @returns {string[]}
 */
export function aacHiddenButtons(resModel) {
    const rules = aacRules();
    if (rules.empty || !resModel) {
        return [];
    }
    const m = rules.buttons_by_model && rules.buttons_by_model[resModel];
    return Array.isArray(m) ? m : [];
}

/**
 * Hidden notebook page titles (page @string) for this model.
 * @param {string} resModel
 * @returns {string[]}
 */
export function aacHiddenTabs(resModel) {
    const rules = aacRules();
    if (rules.empty || !resModel) {
        return [];
    }
    const m = rules.tabs_by_model && rules.tabs_by_model[resModel];
    return Array.isArray(m) ? m : [];
}

/**
 * Robustly extract a numeric ID from a sidebar menu item (Odoo 16 format).
 * @param {object} item
 * @returns {number|null}
 */
export function extractActionId(item) {
    if (!item) return null;
    // Odoo 16 often has direct id
