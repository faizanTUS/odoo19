...
class HrJobShare(models.Model):
    ...
    # job_skill = fields.Char('Job Skill')
    ...
    def _send_linkedin_post(self):
        ...
        for job in jobs:
            ...
            elif job.post_type == 'job_post':
                for user in job.user_ids:
                    job.post_job_to_linkedin(user)

    # def post_job_to_linkedin(self):
    #     """ Method to post a job to LinkedIn """
    #     credential_dict = self.get_authorize(self.env.user)
    #     access_token = credential_dict['access_token']
    #     linkedin_credential = credential_dict['linkedin_credential']
    #
    #     # Define your company ID from LinkedIn (replace this with the actual ID)
    #     # company_id = 'urn:li:organization:86wn157xgugy0a'
    #
    #     # Prepare job posting data (from Odoo fields)
    #     post_data = {
    #         "elements": [
    #             {
    #                 # "integrationContext": f"urn:li:organization:{company_id}",
    #                 "integrationContext": f"urn:li:organization:techultrasolutions" + linkedin_credential['profile_urn'],
    #                 "companyApplyUrl": "https://www.techultrasolutions.com/",
    #                 "externalJobPostingId": f"job_{self.id}",  # Use a unique ID
    #                 "jobPostingOperationType": "CREATE",
    #                 "title": self.job_title,  # Job title from Odoo
    #                 "description": self.description,  # Job description from Odoo
    #                 "listedAt": 1760531891000,  # Use a timestamp for job listing (this is an example)
    #                 "location": self.job_location,  # Job location from Odoo
    #                 "categories": ["rsch"],  # Job category
    #                 "industries": ["urn:li:industry:55"],  # Industry (this is an example)
    #                 # "skillsDescription": "Skills required: Python, Django",  # Skills for the job
    #                 "basicSkills": ["Java", "Spring Boot", "Kafka"],
    #                 "preferredSkills": ["Kubernetes", "Microservices"],
    #                 "companyJobCode": f"company_{self.id}",
    #                 "employmentStatus": "FULL_TIME",  # Employment type
    #                 "experienceLevel": "EXECUTIVE",  # Experience level
    #                 # "trackingPixelUrl": "http://www.trackingpixellocation.com/pixel.gif",  # Tracking pixel URL
    #                 # "listingType": "BASIC"
    #             }
    #         ]
    #     }
    ...
    #     url = "https://api.linkedin.com/rest/jobRequisitions"
    #
    #     try:
    #         # response = requests.post(url, headers=headers, data=json.dumps(post_data))
    #         response = self.share_request('POST' ,url, access_token, data=json.dumps(post_data))
    #
    #         # Check the response
    #         if response.status_code == 201:
    #             # Job posted successfully
    #             # self.linkedin_posted = True
    #             return {'type': 'ir.actions.act_window_close'}  # Close the window or refresh the form
    #         else:
    #             raise UserError(f"Error posting the job to LinkedIn. Status Code: {response.status_code}")
    ...
    def share_linkedin_image(self, user=False):
        ...
        if not attachments:
            raise UserError("Please Attach the Image File Before Sharing.")
        if attachments:
            filepath = '/tmp/'
