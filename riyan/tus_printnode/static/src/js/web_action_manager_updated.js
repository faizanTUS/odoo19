...
import {PdfOptionsModal} from "./Pdf_Options_Modal";
...
//                actionFlags: action.flags,
    ...
    function _getViewInfo(view, action, views, props = {}) {
        ...
        const openFormView = (resId, { activeIds, readonly, newWindow } = {}) => {
            if (target !== "new") {
                if (_getView("form")) {
                    return switchView("form", { readonly, resId, resIds: activeIds }, { newWindow });
                } else {
                    return doAction(
                        {
                            type: "ir.actions.act_window",
...
//            if (action.target !== "new") {
    ...
    //functionality for check  printer is online or not if printer is online then it will be printed and if it is not then it will Simply download the pdf in system
...
//        debugger;
    ...
    async function _triggerDownloadcopy(action, options, type) {
        const url = _getReportUrl(action, type);
        var self = this;
        var reportUrls = _getReportUrl(action, type);
        try {
            await download({
                url: "/report/download",
                data: {
    ...
    async function _triggerDownload(action, options, type) {
        ...
        const url = _getReportUrl(action, type);
        let removeDialog;
        let {print_type, report_type} = action;
        if (report_type !== "qweb-pdf" || print_type === "download") return false;
        if (!print_type) {
            await rpc("/web/dataset/call_kw", {
                    model: "ir.actions.report",
                    method: "printnode_print_action_for_report",
                    args: [null, action.report_name, options["value_number"]],
                    kwargs: {},
                })
                .then(async function (print_action) {
                    debugger;
//                    !print_action.is_online
                    if (
//                    print_action.is_online == true &&
//                    print_action &&
                    print_action.action === "server"
//                      print_action.action === "user_default"
                    ) {
                        print_type = await new Promise((resolve) => {
                        ...
                        removeDialog();
                        debugger;
                        if (print_type === "close") window.location.reload();
                        if (print_type === "download") return false;
                        if (
                            ...
                            var value_number =
                                document.getElementsByName("number")[0].value;
                            options["value_number"] = value_number;
                        }
                        removeDialog();
                    }
                });
            ...
            if (type === "pdf" || type === "text") {
                rpc("/web/dataset/call_kw", {
                    ...
                    .then(function (print_action) {
                        if (
                            !print_action.is_color_printer &&
                            print_action.is_qweb_color_print &&
                            print_action.is_online &&
                            print_action &&
                            print_action.action === "server"
                        ) {
                            env.services.ui.unblock();
                            const msg = sprintf(
                                "This report is color printed. Please Select color printer %s",
                            ...
                            env.services.ui.unblock();
                            const msg = sprintf(
                                "This report is not color printed. Please select normal printer %s",
                                print_action.printer_name
                            );
                            env.services.notification.add(msg, {
                                sticky: false,
...
//                            print_action.is_qweb_color_print &&
                            print_action.is_online == true &&
                            print_action &&
                            (print_action.action === "server" || print_action.action === "user_default")
                        ) {
                            var data = action.data || {};
                            data["value_number"] = options["value_number"];
                            rpc("/web/dataset/call_kw", {
                                    model: "ir.actions.report",
                                    method: "direct_document_print",
                                    args: [
                                        action.id || print_action.report_id,
                                        action.report_name,
                                ...
                                .then(function () {
                                    env.services.ui.unblock();
                                    const msg = sprintf(
                                        "Document sent to the printer for color print %s",
                                        print_action.printer_name
                                    );
                                    env.services.notification.add(msg, {
                                        sticky: false,
                            ...
                            rpc("/web/dataset/call_kw", {
                                ...
                                .then(function (result) {
                                    env.services.ui.unblock();
                                    const msg = sprintf(
                                        "Document sent to the printer %s",
                                        print_action.printer_name
                                    );
                                    env.services.notification.add(msg, {
                                        sticky: false,
                            ...
                            print_action.is_online == false &&
                            print_action &&
                            print_action.action === "server"
                        ) {
                            env.services.ui.unblock();
                            const msg = sprintf(
                                "Please connect to this printer %s",
                                print_action.printer_name
                            );
                            env.services.notification.add(msg, {
                                sticky: false,
                            ...
                            return _triggerDownloadcopy(action, options, type);
                        }
                    });
            } else {
                return _triggerDownloadcopy(action, options, type);
    ...
    async function _executeReportAction(action, options) {
        ...
        if (
            ...
            var value_number = document.getElementsByName("number")[0].value;
            options["value_number"] = value_number;
        }
        if (action.report_type === "qweb-html") {
            ...
            const state = await rpc("/report/check_wkhtmltopdf");
