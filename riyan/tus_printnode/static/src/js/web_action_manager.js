...
//import {PdfOptionsModal} from "@tus_printnode/static/src/js/Pdf_Options_Modal";
...
function makeActionManager(env) {
    ...
    function _preprocessAction(action, context = {}) {
        ...
        if (action.type === "ir.actions.act_window") {
            action.views = [
                ...action.views.map((v) => [v[0], v[1] === "tree" ? "list" : v[1]]),
            ]; // manipulate a copy to keep cached action unmodified
            action.controllers = {};
...
//        debugger;
    ...
    function _getViewInfo(view, action, views, props = {}) {
        ...
        viewProps.noBreadcrumbs =
            "no_breadcrumbs" in action.context
                ? action.context.no_breadcrumbs
                : target === "new";
    ...
    async function _executeActWindowAction(action, options) {
        ...
        if (env.isSmall) {
            ...
            if (lazyView) {
                lazyView =
                    _findView(views, lazyView.multiRecord, action.mobile_view_mode) ||
                    lazyView;
            }
        }
...
//            const actionParams = _getActionParams();
    ...
    function pushState(controller) {
        ...
        if (action.type === "ir.actions.act_window") {
            ...
            newState.id =
                props.resId || (props.state && props.state.resId) || undefined;
        }
