...
class SaleOrderDiscount(models.TransientModel):
    ...
    def prepare_discount_line_values(self, product, amount, taxes, description=None):
        ...
        # Check the content of taxes
        if taxes:
            print(f"Taxes: {taxes.ids}")  # Add logging to check
        else:
            print("No taxes found!")

        if taxes:
            taxes_to_set = [Command.set(taxes.ids)]
        else:
            taxes_to_set = []
        ...
        vals = {
            ...
            'sequence': 999,
            'price_unit': -amount,
            'tax_ids': taxes_to_set,  # Change taxes_id to tax_ids here
        }
        if description:
