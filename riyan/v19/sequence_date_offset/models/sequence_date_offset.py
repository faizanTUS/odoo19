...
def get_interpolation_dates(env, date=None, date_range=None):
    now = range_date_value = effective_date = datetime.now(env.tz)
    if date or env.context.get("ir_sequence_date"):
        effective_date = fields.Datetime.to_datetime(date or env.context.get("ir_sequence_date"))
    if date_range or env.context.get("ir_sequence_date_range"):
        range_date_value = fields.Datetime.to_datetime(
