    ...
    def _check_targets(self):
        for pol in self:
            if pol.active and not pol.user_ids and not pol.group_ids:
                raise ValidationError(_("An active policy must include at least one user or one group."))
...
class AdvancedAccessPolicyModelLine(models.Model):
    ...
    policy_id = fields.Many2one("advanced.access.policy", required=True, ondelete="cascade")
    ...
    def _check_hidden_reports_model(self):
        for line in self:
            ...
            if bad:
                raise ValidationError(_("Hidden reports must belong to model %(model)s.") % {"model": line.model_name})
    ...
    def _check_hidden_sidebar_actions_model(self):
        for line in self:
            if not line.model_id:
                continue
            bad = line.hidden_sidebar_action_ids.filtered(lambda a: a.binding_model_id != line.model_id)
            if bad:
    ...
    def _check_field_exists(self):
        ...
        for line in self:
            ...
            if line.ir_field_id:
                ...
                continue
            f = IrModelFields.search([("model_id", "=", line.model_id.id),("name", "=", line.field_name),], limit=1,)
            if not f:
                raise ValidationError(
...
class AdvancedAccessPolicyMenuLine(models.Model):
    ...
    def _compute_aac_menu_candidate_ids(self):
        for line in self:
            if line.model_id:
                line.aac_menu_candidate_ids = line._aac_menus_for_model(line.model_id.model)
            else:
