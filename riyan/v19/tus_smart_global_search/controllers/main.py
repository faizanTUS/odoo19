...
class GlobalSearchController(http.Controller):
    ...
    @http.route("/tus_smart_global_search/search", type="jsonrpc", auth="user")
    def search(self, query, limit_per_model=10, max_models=50):
        ...
        for rec in model_recs:
            ...
            try:
                ...
                _logger.debug("tus_smart_global_search: name_search failed for %s — %s", model_name, e)
                continue
    ...
    @http.route("/tus_smart_global_search/config", type="jsonrpc", auth="user")
    def get_config(self):
        user = request.env.user
        return {
