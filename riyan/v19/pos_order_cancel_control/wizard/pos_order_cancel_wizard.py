...
class PosOrderCancelWizard(models.TransientModel):
    ...
    def _cancel_done_picking_impl(self, picking, StockQuant):
        ...
        for move in picking.move_ids.filtered(lambda m: m.state == 'done' and not m.scrap_id):
    ...
    def action_confirm(self):
        ...
        if self.cancel_delivery_order:
            ...
            if orders:
                origins = orders.mapped('name')
                pickings_via_origin = self.env['stock.picking'].search([
                    ('origin', 'in', origins),
                    ('state', '!=', 'cancel'),
                ])
                pickings_via_group |= pickings_via_origin
        ...
        # 1) Cancel related invoices
        if invoices_to_cancel:
            for inv in invoices_to_cancel:
                try:
                    _logger.info("Trying to cancel invoice %s (state: %s)", inv.name, inv.state)
                    inv.sudo().button_cancel()
                    _logger.info("Standard cancel succeeded for %s", inv.name)
                except Exception as e:
                    _logger.warning("Standard cancel failed for %s: %s → trying force", inv.name, str(e))
                    try:
                        inv.action_force_cancel()
                        _logger.info("Force cancel succeeded for %s", inv.name)
                    except Exception as force_e:
                        _logger.error("Force cancel also failed for %s: %s", inv.name, force_e, exc_info=True)
                        # Do not raise → continue wizard

                # Clean link anyway
                orders.filtered(lambda o: o.account_move == inv).sudo().write({'account_move': False})
        ...
        # 2b) Payment reversals (optional & isolated)
        if self.cancellation_method in ('cancel_only', 'reset_draft'):
            ...
            if orders_with_payments:
                try:
                    ...
                    _logger.error("Payment reversal failed (isolated): %s", rev_e, exc_info=True)
                    pass  # continue
        ...
        if pickings_cancelled:
            ...
            return {
                ...
                'params': {
                    'type': 'success',
                    'sticky': False,
                    'message': _('Delivery order(s) set to Cancelled: %(names)s.') % {'names': names},
                    'next': {'type': 'ir.actions.act_window_close'},
                },
            }

        if pickings_failed_reasons:
            details = '; '.join('%s: %s' % (n, r) for n, r in pickings_failed_reasons)
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'type': 'warning',
                    'sticky': True,
                    'message': _('Some delivery orders could not be cancelled: %(details)s.') % {'details': details},
                    'next': {'type': 'ir.actions.act_window_close'},
                },
            }
        ...
        if pickings_failed:
            ...
            return {
                ...
                'params': {
                    'type': 'warning',
                    'sticky': False,
                    'message': _('%(count)s delivery order(s) could not be cancelled: %(names)s.') %
