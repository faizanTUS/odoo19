...
class DynamicWarningRule(models.Model):
    ...
    def _rule_applies_for_user(self, rule):
        ...
        if not rule.group_ids:
            return True
        return bool(rule.group_ids & self.env.user.groups_id)
    ...
    #     """
    #     Return list of warnings to display for the given record.
    #
    #     Called from the form view JS with the current record's model and id
    #     (e.g. model_name='product.product', res_id=26 for product variant 26).
    #
    #     Returns a list of dicts: [{'message': str, 'type': 'info'|'warning'|'danger'}, ...]
    #     Only rules whose Target Model matches model_name are evaluated.
    #     """
    #     _logger.warning("MODEL: %s | RES_ID: %s", model_name, res_id)
    #
    #     try:
    #         if not res_id or not model_name:
    #             return []
    #         Model = self.env.get(model_name)
    #         if not Model or not isinstance(Model, type) or not issubclass(Model, models.Model):
    #
    #             return []
    #         record = Model.browse(res_id).exists()
    #         if not record:
    #             return []
    #         # Collect all domain field names so we can pre-load computed fields (e.g. qty_available)
    ...
    #         if not rules:
    #             _logger.info(
    #                 "Dynamic Warnings: get_warnings_for_record(%s, %s) -> 0 rules for model (no rules defined for this model)",
    #                 model_name, res_id,
    #             )
    #             return []
    #         for rule in rules:
    #             domain = rule._get_domain()
    #             for f in self._get_domain_field_names(domain):
    #                 all_domain_fields.add(f)
    #         # Pre-load domain fields so computed/stored fields are evaluated (e.g. qty_available on product)
    #         if all_domain_fields:
    #             valid_fields = [f for f in all_domain_fields if f in record._fields]
    #             if valid_fields:
    #                 try:
    #                     record.read(valid_fields)
    #                 except Exception:
    #                     pass
    #         result = []
    #         for rule in rules:
    ...
    #                 _logger.info(
    #                     "Dynamic Warnings: rule '%s' (id=%s) skipped: user not in allowed groups",
    #                     rule.name, rule.id,
    #                 )
    #                 continue
    #             if not self._rule_valid_dates(rule):
    #                 _logger.info(
    #                     "Dynamic Warnings: rule '%s' (id=%s) skipped: outside valid date range",
    #                     rule.name, rule.id,
    #                 )
    #                 continue
    #             if rule.company_id:
    #                 rec_company = getattr(record, "company_id", None) and record.company_id
    #                 if rec_company and rec_company != rule.company_id:
    #                     _logger.info(
    #                         "Dynamic Warnings: rule '%s' (id=%s) skipped: company mismatch",
    #                         rule.name, rule.id,
    #                     )
    #                     continue
    #                 if not rec_company and self.env.company != rule.company_id:
    #                     _logger.info(
    #                         "Dynamic Warnings: rule '%s' (id=%s) skipped: company mismatch",
    #                         rule.name, rule.id,
    #                     )
    ...
    #                     _logger.info(
    #                         "Dynamic Warnings: rule '%s' (id=%s) skipped: record does not match domain %s",
    #                         rule.name, rule.id, domain,
    #                     )
    #                     continue
    #             message = self._replace_placeholders(rule.message, record)
    #             result.append({
    #                 "message": message,
    #                 "type": rule.warning_type,
    #             })
    #         _logger.info(
    #             "Dynamic Warnings: get_warnings_for_record(%s, %s) -> %d rules for model, %d warnings returned",
    #             model_name, res_id, len(rules), len(result),
    #         )
    #         if result:
    #             _logger.info(
    #                 "Dynamic Warnings: returning %s (first message: %s)",
    #                 result,
    #                 result[0].get("message", "")[:50] if result else "",
    #             )
    #         return result
    #     except Exception as e:
    #         _logger.warning(
    #             "Dynamic Warnings: get_warnings_for_record(%s, %s) failed: %s",
    #             model_name, res_id, e,
    #             exc_info=True,
    #         )
    #         return []
