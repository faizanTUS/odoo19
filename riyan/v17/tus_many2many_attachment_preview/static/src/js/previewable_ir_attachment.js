...
export class PreviewableIrAttachment extends PreviewFileBase {
    constructor(vals) {
        ...
        // Normalize access token naming (Odoo uses accessToken in JS models, ORM gives access_token).
        if (!this.accessToken && this.access_token) {
    ...
    get defaultSource() {
        ...
        if (this.isText && (this.extension === 'csv' || this.mimetype === 'text/csv')) {
            ...
            const token = this.accessToken || this.access_token;
        ...
        if (this.isMsOffice && officePreviewEnabled()) {
            ...
            // console.log("MAP2 DEBUG - URL for Office:", absoluteUrl, "Token:", token);
