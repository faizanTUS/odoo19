# Many2Many Attachment Preview — Step-by-Step Configuration (Odoo 17)

Module location (this repository):

`odoo17/project/tus_many2many_attachment_preview/`
...
1. Confirm the module folder exists on disk, for example:  
   `/home/tus/workspace/odoo17/project/tus_many2many_attachment_preview/`
...
3. Locate **`addons_path`**. Add the **`project`** directory (the parent folder that contains `tus_many2many_attachment_preview`), **not** only the module folder.
   ...
   ```ini
   [options]
   addons_path = /home/tus/workspace/odoo17/odoo/addons,/home/tus/workspace/odoo17/project
   ```

   Adjust paths to match your server. The important part is that **`.../odoo17/project`** is listed so Odoo can discover **`tus_many2many_attachment_preview`** inside it.
