...
# -*- coding: utf-8 -*-
"""Odoo tests for list_kanban_auto_refresh.

Run (paths below match this workspace; pick a PostgreSQL database — e.g. ``test_lkar_refresh_17``
for a “17”-named test DB). Use ``-i`` if the module is not installed yet, ``-u`` if it is::

    /usr/bin/python3.10 /home/tus/workspace/odoo18/odoo/odoo-bin \\
        -c /home/tus/workspace/odoo18/conf/app_18c.conf \\
        -d test_lkar_refresh_17 \\
...
First-time install on an empty database: replace ``-u`` with ``-i list_kanban_auto_refresh``.
