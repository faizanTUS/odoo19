...
class FaceAttendanceController(http.Controller):
    ...
    def check_in_out(self, image_data, latitude=None, longitude=None):
        try:
            ...
            # Check radius validation if configured - validate location BEFORE processing
            # company = request.env.user.company_id
            # radius = getattr(company, 'face_attendance_radius', 0)
            # if radius and radius > 0:
            #     # If radius is configured, validate location first
            #     if not latitude or not longitude:
            #         return {"success": False, "msg": "❌ Location (latitude/longitude) is required for attendance"}
            #
            #     # Get company location
            #     company_lat = getattr(company, 'face_attendance_latitude', False)
            #     company_lon = getattr(company, 'face_attendance_longitude', False)
            #
            #     if not company_lat or not company_lon:
            #         return {"success": False, "msg": "❌ Company location (latitude/longitude) is not configured. Please contact administrator"}
            #
            #     # Calculate distance between employee location and company location
            ...
            #
            #     if distance is None:
            #         return {"success": False, "msg": "❌ Invalid location coordinates"}
            #
            #     # Check if within radius (radius is in meters)
            #     # Only allow login/attendance if within configured radius
            #     if distance > radius:
            #         return {
            #             "success": False,
            #             "msg": f"❌ Location verification failed. You are {distance:.0f} meters away from the office location. Please be within {radius:.0f} meters to mark attendance."
            #         }
