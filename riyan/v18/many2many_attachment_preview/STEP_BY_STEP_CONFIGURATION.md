...
`odoo18/project/many2many_attachment_preview/`
...
1. Confirm the module folder exists on disk, for example:  
   `/home/tus/workspace/odoo18/project/many2many_attachment_preview/`
2. Open your Odoo configuration file (e.g. `odoo.conf`).
3. Locate **`addons_path`**. Add the **`project`** directory (the parent folder that contains `many2many_attachment_preview`), **not** only the module folder.
   ...
   Adjust paths to match your server. The important part is that **`.../odoo18/project`** is listed so Odoo can discover **`many2many_attachment_preview`** inside it.
...
2. Search for **Many2Many Attachment Preview** (technical name: `many2many_attachment_preview`).
...
| Error when clicking the eye | Hard-refresh assets / upgrade `many2many_attachment_preview`; avoid duplicate **FileViewer** patches from another module. |
