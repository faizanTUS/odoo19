...
patch(CogMenu.prototype, {
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (hasAccess) {
                this.registryItems = await originalRegistryItems.call(this);
            } else {
                this.registryItems = [];
            }
        });
    ...
    get hasItems() {
        ...
        const cogItems = [...actionItems, ...registryItems];
        return cogItems.length > 0 || (this.props.items.print?.length || 0) > 0;
    },
