...
class SortRecord(models.Model):
    ...
    # @api.constrains('sort_line_ids', 'user_ids', 'is_active')
    # def _check_duplicate_model_per_user(self):
    #     for record in self:
    #         model_ids_in_record = []
    #
    #         for line in record.sort_line_ids:
    #             model_id = line.model_id.id
    #             model_name = line.model_id.name
    #
    #             if model_id in model_ids_in_record:
    #                 raise ValidationError(_(
    #                     "Model '%s' is repeated within the same sorting record for user(s): %s"
    #                 ) % (model_name, ', '.join(record.user_ids.mapped('name'))))
    #             model_ids_in_record.append(model_id)
    #
    #             conflicting_lines = self.env['sorting.record.line'].sudo().search([
    #                 ('model_id', '=', model_id),
    #                 ('sort_id', '!=', record.id),
    #                 ('sort_id.user_ids', 'in', record.user_ids.ids),
    #                 ('sort_id.is_active', '=', True),
    #             ])
    #
    #             if conflicting_lines:
    #                 conflict_users = conflicting_lines.mapped('sort_id.user_ids.name')
    #                 raise ValidationError(_(
    #                     "Model '%s' is already used in another active sorting for user(s): %s"
    #                 ) % (model_name, ', '.join(set(conflict_users))))
...
class SortRecordLine(models.Model):
    ...
    user_ids = fields.Many2many(related='sort_id.user_ids', string="User")
...
class BaseModel(models.BaseModel):
    ...
    def web_search_read(self, domain, specification, offset=0, limit=None, order=None, count_limit=None):
        ...
        all_sort_line_id = self.env['sorting.record.line'].sudo().search(
            [('sort_id.is_active', '=', True), ('model_id.model', '=', records._name), ('sort_id.user_ids', 'in', [self.env.uid])])
        if all_sort_line_id:
    ...
    def _name_search(self, name, domain=None, operator='ilike', limit=None, order=None):
        ...
        all_sort_line_id = self.env['sorting.record.line'].sudo().search([('sort_id.is_active', '=', True), ('model_id.model', '=', self._name), ('sort_id.user_ids', 'in', [self.env.uid])])
        if all_sort_line_id:
