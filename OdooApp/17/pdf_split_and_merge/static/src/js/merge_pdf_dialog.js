...
export class MergePdfDialog extends Component {
    ...
    async loadData() {
        try {
            const result = await this.rpc("/pdf_merge/load_data", {
    ...
    async onFileUpload(e) {
        ...
        reader.onload = async (e) => {
            try {
                const result = await this.rpc("/pdf_merge/process_uploaded_file", {
                    file_name: file.name,
        ...
        try {
            const pagesWithContent = await Promise.all(
                selectedPages.map(async (page) => {
                    ...
                    const result = await this.rpc("/pdf_merge/get_page_content", {
                        document_id: this.props.action.props.documentId,
                        page_id: page.id,
                        page_type: page.type,
                        page_number: page.page_number
            ...
            const result = await this.rpc("/pdf_merge/merge_pages", {
                document_id: this.props.action.props.documentId,
