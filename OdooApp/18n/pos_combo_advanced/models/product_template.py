...
class ProductTemplate(models.Model):
    ...
    def action_set_type_service(self):
        ...
        self.ensure_one()
        self.write({'type': 'service'})
        return True

    def action_view_combo_items(self):
        """Open Combo Items (product.combo.item) for current product's combos."""
        self.ensure_one()
        if not self.combo_ids:
            return False
        combo_ids = self.combo_ids.ids
        return {
            'type': 'ir.actions.act_window',
            'name': 'Combo Items',
            'res_model': 'product.combo.item',
            'view_mode': 'list',
            'domain': [('combo_id', 'in', combo_ids)],
            'context': {'default_combo_id': combo_ids[0] if combo_ids else False},
        }
