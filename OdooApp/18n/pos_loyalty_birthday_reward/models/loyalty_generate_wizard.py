...
class LoyaltyGenerateWizard(models.TransientModel):
    ...
    def _cron_generate_birthday_rewards(self):
        ...
        for partner in birthday_partners:
            for program in birthday_programs:
                ...
                loyalty_program = self.env['loyalty.card'].search(
                    [('partner_id', '=', partner.id), ('program_id', '=', program.id)])
                lp = loyalty_program.filtered(lambda p: fields.Date.from_string(
                    p.partner_id.birthdate).month == date.month and fields.Date.from_string(
                    p.create_date).year == date.year)
                if not lp:
                    wizard = self.create({
    ...
    def _send_birthday_notifications(self):
        ...
        template = self.env.ref('_send_birthday_notifications.birthday_reward_email_template', raise_if_not_found=False)
        if not template:
            return
