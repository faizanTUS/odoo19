...
class Base(models.AbstractModel):
    ...
    def _check_access(self, operation):
        ...
        if result or self.env.su:
            return result
        if not self._aac_should_enforce():
            return None
        ...
        if denied:
            svc._audit_denial(model, operation,"policy model rule (%s records)" % len(denied),)
            return denied, functools.partial(
                AccessError,
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        ...
        if caps:
            ...
            elif view_type == "list" and root.tag in ("list", "tree"):
                if not caps.get("write", True):
                    root.set("edit", "false")
                if not caps.get("create", True):
                    root.set("create", "false")
        ...
        if view_type == "list" and root.tag in ("list", "tree") and not caps:
