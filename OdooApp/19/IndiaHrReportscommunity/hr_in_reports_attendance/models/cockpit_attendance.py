...
class AttendanceCockpitProvider:
    ...
    def collect(env, filters):
        ...
        day_rows = Att.formatted_read_group(
            base_dom,
            ["date:day"],
            ["worked_hours:sum"],
            order="date:day",
        )
        count_rows = Att.formatted_read_group(
            base_dom,
            ["date:day"],
            ["__count"],
            order="date:day",
        ...
        hours_by_day = {_norm_day_key(r): float(r.get("worked_hours:sum", 0) or 0) for r in day_rows}
