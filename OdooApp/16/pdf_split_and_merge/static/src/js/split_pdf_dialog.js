...
export class SplitPdfDialog extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            try {
                const result = await this.rpc("/pdf_split/load_pages", {
    ...
    async splitSelectedPages() {
        ...
        const result = await this.rpc("/pdf_split/split_selected", {
            document_id: this.props.action.props.documentId,
        ...
        if (result.success) {
            ...
            this.notification.add(result.error || "Split failed", { type: "danger" });
        }
    }
}
