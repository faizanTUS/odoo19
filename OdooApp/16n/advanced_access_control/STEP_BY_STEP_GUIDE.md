# Advanced Access Control and UI Restrictions Manager | Hide Chatter, Developer Mode, Buttons, Menus and Actions — Step-by-step guide (Odoo 16)

**Module technical name:** `advanced_access_control`  
**Product title (SEO):** Advanced Access Control and UI Restrictions Manager | Hide Chatter, Developer Mode, Buttons, Menus and Actions

This guide walks you from installation through common configuration patterns. It also includes keywords useful for
internal documentation or marketplace listings (Odoo 16, access rights manager, field-level security, hide menus,
...
1. Copy or clone the folder `advanced_access_control` into an addons directory that your Odoo 16 server already loads (
...
1. Enable **Developer mode** (Settings → scroll to bottom → Activate the developer mode), or start Odoo with a user that
   can install apps.
2. Go to **Apps**, remove the *Apps* filter, search for **Advanced Access Control and UI Restrictions Manager | Hide Chatter, Developer Mode, Buttons, Menus and Actions** (or the technical name above).
3. Click **Install**.
...
- **When (expression)** (optional): a client-side expression like `state == 'done'` (same family as dynamic view
  attributes in Odoo 16). If empty, the modifier is always applied (`1`).
