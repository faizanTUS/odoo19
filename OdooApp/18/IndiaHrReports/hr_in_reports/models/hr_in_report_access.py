...
class HrInReportAccess(models.AbstractModel):
    ...
    def has_reports_access(self):
        ...
        user = self.env.user
        if user.has_group("IndiaHrReports.group_hr_in_reports_user"):
            return True
        if user.has_group("IndiaHrReports.group_hr_in_reports_manager"):
            return True
        return any(
            user.has_group(g)
            for g in (
    ...
    def is_hrms_manager(self):
        ...
        user = self.env.user
        return (
            user.has_group("IndiaHrReports.group_hr_in_reports_manager")
