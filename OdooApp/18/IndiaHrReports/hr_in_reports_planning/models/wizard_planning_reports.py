...
class HrInReportWizardPlanCoverage(models.TransientModel):
    ...
    def _pdf_report_xmlid(self):
        return "IndiaHrReports.action_report_plan_coverage"
    ...
    def _get_dataset(self):
        ...
        for s in slots:
            ...
            rows.append(
                {
                    "resource": res_name or self.env._("Open / unassigned"),
                    "employee": emp,
                    "department": dept,
                    "role": role,
...
class HrInReportWizardPlanUnderOver(models.TransientModel):
    ...
    def _pdf_report_xmlid(self):
        return "IndiaHrReports.action_report_plan_under_over"
    ...
    def _get_dataset(self):
        ...
        for rid, planned in sorted(by_res.items(), key=lambda x: (-x[1], x[0])):
            if rid:
                ...
                label = res.name or self.env._("Resource #%s") % rid
            else:
                label = self.env._("Open / unassigned shifts")
            rows.append(
                {
                    "resource": label,
