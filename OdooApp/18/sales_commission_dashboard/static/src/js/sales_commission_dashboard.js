...
export class SalesCommissionDash extends Component {
    ...
    async loadSalesCommissionData() {
        ...
        try {
            ...
            const commissionRecords = await this.orm.call("sale.commission.report", "read_group", [
                domain,
    ...
    renderSalesCommissionTable() {
        try {
            if (!this.commissionSummaryBody.el) {
            ...
            if (this.state.salesCommissionData.length === 0) {
                tbody.innerHTML =
                    '<tr><td colspan="4" class="text-center text-muted">No commission data available</td></tr>';
                return;
            }
            for (const row of this.state.salesCommissionData) {
