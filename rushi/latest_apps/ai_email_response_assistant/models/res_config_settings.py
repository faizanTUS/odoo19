...
class ResConfigSettings(models.TransientModel):
    ...
    count = fields.Integer(string='Count', default='5')
    ...
    def _check_count(self):
        for rec in self:
            if rec.count > 15:
                raise ValidationError("No. of emails cannot be greater than 15.")

    @api.model
    def get_values(self):
        res = super(ResConfigSettings, self).get_values()
        ICPSudo = self.env['ir.config_parameter'].sudo()
        count = int(ICPSudo.get_param('mail.compose.message.count', default=5))
        ...
        res.update(count=count,select_llm_model=select_llm_model, llm_model_key=llm_model_key)
        return res

    def set_values(self):
        super(ResConfigSettings, self).set_values()
        ICPSudo = self.env['ir.config_parameter'].sudo()
        ICPSudo.set_param('mail.compose.message.count', self.count)
